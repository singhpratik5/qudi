...
class POIExtractor:
    ...
    **config
    ...
    def _narrow_candidates(self, candidates, zone_stats, pixel_size_um, cfg, diag=None):
        ...
        if method == 'otsu' and len(scores) >= 2:
            max_score, min_score = np.max(scores), np.min(scores)
            if max_score - min_score < 0.15:
                # Scores are very tight, accept all
                score_threshold = float(min_score - 1e-5)
            elif len(scores) >= 4:
                # Max-Gap split
                sorted_scores = np.sort(scores)
                gaps = np.diff(sorted_scores)
                max_gap_idx = np.argmax(gaps)
                score_threshold = float(sorted_scores[max_gap_idx] + gaps[max_gap_idx] / 2.0)
                if diag is not None:
                    diag['max_gap_split_used'] = True
            else:
